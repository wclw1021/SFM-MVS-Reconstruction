...
if any(tr2add)
    ...
    % 确保右侧的数据大小与左侧的索引范围匹配
    newObsVal = GraphB.ObsVal(:, ids(nonZerosID));
    GraphAB.ObsVal(:, (curValCnt + 1):(curValCnt + length(nonZerosID))) = newObsVal;

    idsNew = zeros(size(ids));
    temp = (curValCnt + 1):(curValCnt + length(nonZerosID));
    
    % 这里这么写是因为有一个莫名其妙的bug
    if size(temp, 1) < size(temp, 2)
        idsNew(nonZerosID) = temp';
    else
        idsNew(nonZerosID) = temp;
    end

    % 确保左侧和右侧的大小匹配
    newObsIdx = sparse(idsNew);
    newObsIdxRows = length(GraphA.frames) + 1:length(GraphA.frames) + length(indexNewFramesFromB);
    newObsIdxCols = size(GraphAB.ObsIdx, 2) + 1:size(GraphAB.ObsIdx, 2) + size(newObsIdx, 2);
    GraphAB.ObsIdx(newObsIdxRows, newObsIdxCols) = newObsIdx;

    % 确保左侧和右侧的大小匹配
    newStrCols = size(GraphAB.Str, 2) + 1:size(GraphAB.Str, 2) + size(GraphB.Str(:, tr2add), 2);
    GraphAB.Str(:, newStrCols) = GraphB.Str(:, tr2add);
end
return;
